...
import { ArrowLeft, Activity, ShieldAlert, Maximize2, Camera, AlertOctagon } from 'lucide-react'
...
export default function IntrusionDetails() {
  const { id } = useParams()
  ...
  // Intrusion Detection specific styling overrides
  const config = { label: 'Intrusion Detection', emoji: '🚨', color: '#ef4444', route: 'intrusion' }

  const getMockThumb = (type) => 'https://images.unsplash.com/photo-1555529733-0e67056058ab?auto=format&fit=crop&q=80&w=300&h=200'
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        if (rawDets.length > 0 && Math.random() > 0.6 && Object.keys(payload).length < 3) {
    ...
    fetchAnalytics()
    // Intrusion needs highly frequent pinging
          ...
          <div style={{ width: 500, flexShrink: 0, background: '#000', borderRadius: 'var(--radius)', overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow)' }}>
            ...
            <div style={{ background: '#111', padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #333' }}>
               <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ color: '#fff', fontSize: 12, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' }}>INTRUSION | LIVE STREAM</span>
               </div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
             <div style={{ background: '#fff', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 20, flex: 1 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                   <div>
                     <div style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600 }}>ARMED ZONES</div>
                     <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)' }}>3</div>
                   </div>
                   <div>
                     <div style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600 }}>VIOLATIONS (M)</div>
             ...
             <div style={{ background: '#fff', borderRadius: 'var(--radius)', border: '1px solid var(--border)', padding: 20, flex: 1 }}>
               ...
               <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                 ...
                 <div style={{ fontSize: 13, color: 'var(--text-2)', fontWeight: 500, lineHeight: 1.4 }}>
                   Active perimeter breaches physically detected bridging polygon boundaries in view.
                 </div>
               </div>
             </div>
          </div>
           ...
           <div style={{ overflowX: 'auto', maxHeight: 400 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                      ...
                      <tr key={`${ev.track_id}-${idx}`} style={{ borderBottom: '1px solid #e2e8f0', background: '#fff', transition: 'background 0.2s' }}>
                        ...
                        <td style={{ padding: '12px 20px' }}>
                           ...
                           <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, marginTop: 4, textTransform: 'capitalize' }}>
                             Target: {ev.vehicle_type || 'Intruder'}
                           </div>
                        </td>
                        ...
                        <td style={{ padding: '12px 20px', textAlign: 'right' }}>
                           <div style={{ fontSize: 12, fontWeight: 800, color: '#10b981' }}>{ev.confidence ? `${Math.round(ev.confidence * 100)}%` : '99%'} Match</div>
                           <div style={{ fontSize: 10, color: 'var(--text-3)', fontWeight: 600, marginTop: 4 }}>AI Validated</div>
                        </td>
                      </tr>
                    )
                  })}
                  {liveEvents.length === 0 && (
                    <tr>
                      <td colSpan="4" style={{ padding: 40, textAlign: 'center', color: 'var(--text-3)', fontWeight: 500 }}>
                         No telemetry data available. Perimeter is secure.
                      </td>
                    </tr>
                  )}
                </tbody>
